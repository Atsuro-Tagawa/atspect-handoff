...
- P1〜P4全面の指定20箇所以上の文言を差し替え、3箇所を削除（P1「2026.07」／P2「収録は…」／P3市場図の重複キャプション）、P4に「AR-P0　第1刷」を新設。ビットマップ合成PDFのためPIL画像処理で実施、本文書体はNoto Sans JP(weight≈330)に実測近似（完全一致ではない旨明記）。
- **新文言が長い箇所は指示どおりフォント縮小せず複数行へ折り返し**。禁則処理でブランド名・動詞の分断も回避。
- **Codexレビュー（1ファイル1観点・18分割）で実在バグ2件を発見・即是正**＝①P3料金箱見出しが罫線と重なっていた（座標起点ミス）②P2メッセージ文で「できる」が分断されていた。9件PASS・7件はCodexツール側の非結果（制作T自身の実測では確認済み）＝正直に記録。
- verify_before_print.py全PASS・確定タグラインはバイト完全一致を機械確認・対外文言禁止事項0件。デスクトップ確認フォルダを`_文言確定版`として新規作成・MD5一致確認。

## 前便：パンフレット裏表紙にメール・サイトアイコンを追加
