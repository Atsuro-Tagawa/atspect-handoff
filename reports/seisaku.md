...
## 代表作「配列(ja) vs フラット正本masterworks_text」食い違い　裁定材料表（2026-08-22／制作T／Sonnet 5・Codex）

翻訳品質Tの起票（第40束・67便＝「配列(ja)とフラット正本masterworks_textが食い違う＝36名」）を受け、**裁定材料の抽出・分類のみ**を実施（値の是正は一切行っていない）。

### ①実測の食い違い総数（翻訳T報告36名との差）

- **実測65件**（翻訳T報告36名との差＝**+29件**）。
- 内訳の経緯＝1巡目の機械抽出は39件（母集団の取り違え差+3件のみ）だったが、**Codexの独立検証で「配列(masterworks)を持たない作家を一律で比較対象外にしていたため、フラット側にだけ値がある26件を見落としていた」ことが発覚**し、ロジックを修正して65件（39+26）を確定した。
- ★母集団も訂正＝ライブ実測629件のうち2件（`mihon-*`）はCodexの検証で**見本ページ用のダミー（実在の作家ではない）**と判明し除外。**実在の作家627件**が正しい母数。

### ②A/B/C分類の内訳

- **A（補足・表記ゆれのみ／作品同定は一致）＝9件**
- **B（作品名そのものが別）＝27件**
- **C（片方が空）＝29件**（うち26件は「配列自体が存在しない・フラットのみ値あり」の新規発見分、既存3件は「配列はあるがフラットが空／その逆」）
- 分類は機械の一次分類（句読点・括弧の全角半角・助詞「の」等の表記ゆれを吸収して同定一致を判定）。**事実としてどちらの作品名が正しいかの判断は一切していない**（リサーチT／司令塔の持ち場）。

### ③画面に見えている側の内訳

- **配列(ja)側が表示＝39件／フラット側が表示＝26件**（合計65件）。
- テーマ161 `sections/atspect-artist-detail.liquid` の実際のJS（2026-08-20実装・879〜899行）を live 実測（Admin API `theme.files` 取得とローカル作業コピーがバイト完全一致=diff 0行を確認済み）でそのままトレースして判定。配列(ja)に値がある作家はJSがフラットを上書きするため**フラット正本は画面上どこにも表示されない**。配列自体が無い26件（建築家が中心）は**フラット正本がそのまま画面(ja)に表示される**。

### ④Codex検証結果

- **母集団**：保存済みAPIデータ629件はhandle重複なくページネーションにも欠陥なし。
- **既存39件**：独立再計算と一致、明らかな誤分類なし。
- **抽出漏れ**：26件（上記②③に反映済み）。
- **その他確認済み**：JSONパースエラー0件／代表作配列の旧キー名混同なし（`works`キー3件は別データ・masterworksの旧名ではない）／要素順序だけの相違0件。
- **未実施**：Codex実行環境に`SHOPIFY_ADMIN_API_TOKEN`が無く、ライブ再照会（totalCount照会・個別10件の再確認）は今回未完了。

### 実施していないこと／判定していないこと

- **値の是正は一切行っていない**（読み取り・分類のみ）。
- **B分類27件について、どちらの作品名が事実として正しいかの裁定は行っていない**（リサーチT／司令塔の持ち場）。
- Codexの検証観点のうち、ライブAPIへの直接再照会（トークン不在のため）は未実施。かわりに2026-08-20取得済みの全フィールドスナップショット（627件）との突合で代替した。

### 証拠・材料

- 材料表＝`reports/assets/20260822_代表作食い違い材料表/材料表.md`（65件全件・全文・分類根拠つき）
- 全文TSV＝`reports/assets/20260822_代表作食い違い材料表/代表作食い違い_全文.tsv`（Excel可・UTF-8 BOM・CRLF）

最終更新：2026-08-22（代表作食い違い裁定材料表便）

### 参考＝リサーチTが同時並行で建築家の代表作裏取りを実施中（本便のC分類と重複あり）

本便のC分類26件（配列自体が無くフラットのみ値がある建築家群）のうち、**吉阪隆正・今井兼次・東孝光・土浦亀城の4名は、リサーチTが同時刻に`reports/assets/20260822_建築4名裏取り/`で独立に代表作の事実確認（裏取り）を実施中**と判明した（本便のデータ抽出・分類自体はリサーチTの内容を使わず独立に行った。重複の有無を確認するため見出しのみ確認しており、事実確認の結果は転記していない）。**司令塔にて両便の重複作業がないか確認をお願いしたい。**
